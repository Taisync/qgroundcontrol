...
public:
    ...
    const QString baudRate         () { return _baudRate;}
    void setDevFile        (const QString& devFile);
    void setBaudRate       (const QString& baudRate);
    Q_INVOKABLE void changDevFile(QString devFile);
    Q_INVOKABLE void changBaudRate(QString baudRate);
...
public:
    TTYSLink(SharedLinkConfigurationPtr& config);
    virtual ~TTYSLink();
